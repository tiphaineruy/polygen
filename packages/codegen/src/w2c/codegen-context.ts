...
type ImportMap = Record<string, W2CCodegenImportedModule>;
...
export class W2CModuleCodegenContext {
  ...
  public get importedMemories(): GeneratedImport<ModuleMemory>[] {
    return this.imports.filter(
      (i) => i.target.kind === 'memory'
    ) as GeneratedImport<ModuleMemory>[];
  }

  public get importedFunctions(): GeneratedFunctionImport[] {
    return this.imports.filter(
      (i) => i.target.kind === 'function'
    ) as GeneratedFunctionImport[];
  }
 ...
 */
export class W2CCodegenImportedModule {
...
function processImportedModulesInfo(module: Module) {
  ...
  const importsInfo = importedModuleNames
    .values()
    .map((name): [string, W2CCodegenImportedModule] => [
      name,
      new W2CCodegenImportedModule(name),
    ]);
